...
'+box_vec+'MODEL        1\n')
			...
			for bead in atomistic_fragments[residue_type][resid]:
				if int(resid)==0:
					fragment.append('[ '+str(res_counter)+' ]')
					fragment_names.append(str(res_counter))

				res_counter+=1
				if int(resid) == 0:
					frag_index=''
				for at_id, atom in enumerate(atomistic_fragments[residue_type][resid][bead]):
					if int(resid) ==0:
						frag_index+=str(at_counter)+' '
						at_counter+=1	
					if residue_type=='ION':
						if atomistic_fragments[residue_type][resid][bead][atom]['atom'] not in ion_sep and 'SOL' not in atomistic_fragments[residue_type][resid][bead][atom]['res_type']:
							ion_sep[atomistic_fragments[residue_type][resid][bead][atom]['atom']]=0
						if 'SOL' not in atomistic_fragments[residue_type][resid][bead][atom]['res_type']:
							ion_sep[atomistic_fragments[residue_type][resid][bead][atom]['atom']]+=1
						pdb_output.write(pdbline%((int(atom),atomistic_fragments[residue_type][resid][bead][atom]['atom'],atomistic_fragments[residue_type][resid][bead][atom]['res_type'],' ',1,\
						atomistic_fragments[residue_type][resid][bead][atom]['coord'][0],atomistic_fragments[residue_type][resid][bead][atom]['coord'][1],atomistic_fragments[residue_type][resid][bead][atom]['coord'][2],1,0))+'\n')
					else:
						pdb_output.write(pdbline%((int(atom),atomistic_fragments[residue_type][resid][bead][atom]['atom'],atomistic_fragments[residue_type][resid][bead][atom]['res_type'],' ',1,\
						atomistic_fragments[residue_type][resid][bead][atom]['coord'][0],atomistic_fragments[residue_type][resid][bead][atom]['coord'][1],atomistic_fragments[residue_type][resid][bead][atom]['coord'][2],1,0))+'\n')
				if int(resid) == 0:
					fragment.append(frag_index)
...
def reorientate(cg_residue_type,cg_residues):
	...
	for cg_resid, cg_residue in enumerate(cg_residues):
		...
		if cg_residue_type not in ['SOL', 'ION']:
			for cg_bead in cg_residues[cg_residue]:
				...
				for i in at_residues[cg_bead]:
					at_residues[connect[run][0][0]][i]['coord']=rotate_atom(at_residues[connect[run][0][0]][i]['coord'], center, xyz_rot_apply)
		atomistic_fragments[str(cg_resid)]=at_residues
